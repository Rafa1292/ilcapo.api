...
itemPriceRouter.post(
  ...
  async (req: Request, res: Response): Promise<void> => {
    ...
    try {
      const {id, ... newItemPrice} = await itemPriceFactory.toNewItemPrice(req.body);
...
itemPriceRouter.put(
  ...
  async (req: Request, res: Response): Promise<void> => {
    ...
    try {
      ...
      const updatedItemPrice = await itemPriceService.updateItemPrice(updateItemPrice);
      response.setResponse(updatedItemPrice, ["ItemPrice updated successfully"], false);
    } catch (error) {
      errorHandler(error);
    }
    res.send(response);
